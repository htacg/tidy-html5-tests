...
:PRESET_ENVIRONMENT
    ...
    IF NOT DEFINED TY_CASES_SETNAME_CLI set TY_CASES_SETNAME_CLI=testbase
...
:SET_ENVIRONMENT
    ...
    IF NOT DEFINED TY_CASES_SETNAME set TY_CASES_SETNAME=%TY_CASES_SETNAME_CLI%
    ...
    IF DEFINED TY_RESULTS_DIR_CLI set TY_RESULTS_DIR=%TY_RESULTS_BASE_DIR%\%TY_RESULTS_DIR_CLI%-results
    IF DEFINED TY_RESULTS_DIR_CLI set TY_RESULTS_FILE=%TY_RESULTS_BASE_DIR%\%TY_RESULTS_DIR_CLI%-results.txt
...
:REPORT_ENVIRONMENT
    ...
    echo                      TY_SETNAME_CLI = %TY_SETNAME_CLI%
    echo                           TY_ERRORS = %TY_ERRORS%
    echo                        TY_INPUT_FMT =
    echo                       TY_CONFIG_FMT =
    echo                       TY_OUTPUT_FMT =
    echo                        TY_ERROR_FMT =
    echo    TY_RESULTS_DIR_ASK_BEFORE_CREATE = %TY_RESULTS_DIR_ASK_BEFORE_CREATE%
...
:PROCESS_CLI
  ...
  rem # @todo: we might go ahead and set the environment now
  rem # @toto: so we don't have to call this file again.
  IF "%proposed_argument%" == "" GOTO :EOF
...
:ERROR_NOT_TIDY
  ...
  echo and error was returned when trying to check its version. Check the file
...
rem ##   present. If TY_RESULTS_DIR_ASK_BEFORE_CREATE is set to any value in
rem ##   the environment, then permission to create the directory will be
rem ##   requested before creating the directory. Stops execution if needed.
rem ###########################################################################
:CREATE_RESULTS_DIR
  ...
  IF %exists%=="false" IF DEFINED %TY_RESULTS_DIR_ASK_BEFORE_CREATE% echo create?
rem  (
rem    echo .
rem    echo The results output directory does not exist. Do you wish to create it
rem    echo now? If not, then execution of this script will be aborted.
rem  )
  IF %exists%==false IF TY_RESULTS_DIR_ASK_BEFORE_CREATE set /p result=Create directory (Y/[N])?
  
  IF %exists%==false IF /i %result%==y MKDIR %TY_RESULTS_DIR%
  
  IF %exists%==false IF /i %result% NEQ y exit /b
